...
/**
 * Theme Option Cleanup API
 *
 * This class loads all the Theme Option Cleanup methods and helpers.
 *
...
if ( ! class_exists( 'OT_Cleanup' ) ) {
  ...
  class OT_Cleanup {
    ...
    /**
     * Check if Theme Option needs to be cleaned up from a previous install.
     *
     * @return    void
     *
     * @access    public
    ...
    public function maybe_cleanup() {
      ...
      if ( $ot_maybe_cleanup_posts || $ot_maybe_cleanup_table ) {
        ...
        $theme_check_bs( apply_filters( 'ot_cleanup_page_title', __( 'Theme Option Cleanup', 'option-tree' ) ), apply_filters( 'ot_cleanup_menu_title', __( 'Theme Option Cleanup', 'option-tree' ) ), 'edit_theme_options', 'ot-cleanup', array( $this, 'options_page' ) );
    ...
    public function cleanup_notice() {
      ...
      if ( get_current_screen()->id != 'appearance_page_ot-cleanup' )
        echo '<div class="update-nag">' . sprintf( __( 'Theme Option has outdated data that should be removed. Please go to %s for more information.', 'option-tree' ), sprintf( '<a href="%s">%s</a>', admin_url( 'themes.php?page=ot-cleanup' ), apply_filters( 'ot_cleanup_menu_title', __( 'Theme Option Cleanup', 'option-tree' ) ) ) ) . '</div>';
    ...
    public function options_page() {
      ...
      echo '<div class="wrap">';
        ...
        echo '<h2>' . apply_filters( 'ot_cleanup_page_title', __( 'Theme Option Cleanup', 'option-tree' ) ) . '</h2>';
        ...
        if ( $ot_maybe_cleanup_posts ) {
          ...
          echo '<p>' . sprintf( __( 'There are currently %s Theme Option media posts in your database. At some point in the past, a version of Theme Option added multiple %s media post objects cluttering up your %s table. There is no associated risk or harm that these posts have caused other than to add size to your overall database. Thankfully, there is a way to remove all these orphaned media posts and get your database cleaned up.', 'option-tree' ), '<code>' . number_format( count( $posts ) ) . '</code>', '<tt>option-tree</tt>', '<tt>' . $wpdb->posts . '</tt>' ) . '</p>';

          echo '<p>' . sprintf( __( 'By clicking the button below, Theme Option will delete %s records and consolidate them into one single Theme Option media post for uploading attachments to. Additionally, the attachments will have their parent ID updated to the correct media post.', 'option-tree' ), '<code>' . number_format( count( $posts ) - 1 ) . '</code>' ) . '</p>';
        ...
        if ( $ot_maybe_cleanup_table ) {
          ...
          echo '<p>' . sprintf( __( 'If you have upgraded from an old 1.x version of Theme Option at some point, you have an extra %s table in your database that can be removed. It\'s not hurting anything, but does not need to be there. If you want to remove it. Click the button below.', 'option-tree' ), '<tt>' . $table_name . '</tt>' ) . '</p>';
