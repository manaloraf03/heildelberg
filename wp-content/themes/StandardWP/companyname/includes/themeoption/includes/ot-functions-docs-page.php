...
/**
 * Theme Option documentation page functions.
 *
  ...
  function ot_type_creating_options() {
    ...
    echo '<div class="format-setting type-textblock wide-desc">';
      ...
      echo '<div class="description">';
        ...
        echo '<p>' . __( 'Add a comma separated list of post type like <code>post,page</code>. This will only affect the following option types: Custom Post Type Checkbox, & Custom Post Type Select. Below are the default post types available with WordPress and that are also compatible with Theme Option. You can also add your own custom <code>post_type</code>. At this time <code>any</code> does not seem to return results properly and is something I plan on looking into.', 'option-tree' ) . '</p>';
  ...
  function ot_type_option_types() {
    ...
    echo '<div class="format-setting type-textblock wide-desc">';
      ...
      echo '<div class="description">';
        ...
        echo '<p>' . sprintf( __( 'The CSS option type is a textarea that when used properly can add dynamic CSS to your theme from within Theme Option. Unfortunately, due server limitations you will need to create a file named %s at the root level of your theme and change permissions using %s so the server can write to the file. I have had the most success setting this single file to %s but feel free to play around with permissions until everything is working. A good starting point is %s. When the server can save to the file, CSS will automatically be updated when you save your Theme Options.', 'option-tree' ), '<code>dynamic.css</code>', '<code>chmod</code>', '<code>0777</code>', '<code>0666</code>' ) . '</p>';
...
add_filter( \'ot_recognized_font_families\', \'filter_ot_recognized_font_families\', 10, 2 );</code></pre>';
        ...
        echo '<p>' . sprintf( __( 'The Upload option type is used to upload any WordPress supported media. After uploading, users are required to press the "%s" button in order to populate the input with the URI of that media. There is one caveat of this feature. If you import the theme options and have uploaded media on one site the old URI will not reflect the URI of your new site. You will have to re-upload or %s any media to your new server and change the URIs if necessary.', 'option-tree' ), apply_filters( 'ot_upload_text', __( 'Send to Theme Option', 'option-tree' ) ), 'FTP' ) . '</p>';
  ...
  function ot_type_examples() {
    ...
    echo '<div class="format-setting type-textblock wide-desc">';
      ...
      echo '<div class="description">';
        ...
        echo '<p class="aside">' . __( 'If you\'re using the plugin version of Theme Option it is highly recommended to include a <code>function_exists</code> check in your code, as described in the examples below. If you\'ve integrated Theme Option directly into your themes root directory, you will <strong>not</strong> need to wrap your code with <code>function_exists</code>, as you\'re guaranteed to have the <code>ot_get_option()</code> function available.', 'option-tree' ) . '</p>';
...
require( trailingslashit( get_template_directory() ) . \'includes/meta-boxes.php\' );
        ...
        echo '<ul class="docs-ul">';
          echo '<li>' . __( 'Add a variation of the following code to your <code>meta-boxes.php</code>. You\'ll obviously need to fill it in with all your custom array values. It\'s important to note here that we use the <code>admin_init</code> filter because if you were to call the <code>ot_register_meta_box</code> function before Theme Option was loaded the sky would fall on your head.', 'option-tree' ) . '</li>';
        echo '</ul>';
  ...
  function ot_type_theme_mode() {
    ...
    echo '<div class="format-setting type-textblock wide-desc">';
      ...
      echo '<div class="description">';
        ...
        echo '<p>' . __( 'There are a few simple steps you need to take in order to use Theme Option as a theme included module. In the code below I\'ll show you a basic demo of how to include the entire plugin as a module, which will allow you to have the most up-to-date version of Theme Option without ever needing to hack the core of the plugin. If you would like to see some demo code, there is a directory named <code>theme-mode</code> inside the <code>assets</code> directory that contains a file named <code>demo-theme-options.php</code> you can reference.', 'option-tree' ) . '</p>';
        
        echo '<h5>' . __( 'Step 1: Include the plugin & turn on theme mode.', 'option-tree' ) . '</h5>';
        echo '<ul class="docs-ul">';
          echo '<li>' . sprintf( __( 'Download the latest version of %s and unarchive the %s directory.', 'option-tree' ), '<a href="http://wordpress.org/extend/plugins/option-tree/" rel="nofollow" target="_blank">' . __( 'Theme Option', 'option-tree' ) . '</a>', '<code>.zip</code>' ) . '</li>';
...
/**
 * Required: include Theme Option.
 */
require( trailingslashit( get_template_directory() ) . \'option-tree/ot-loader.php\' );
</code></pre>';
        
        echo '<p>' . sprintf( __( 'For a list of all the Theme Option UI display filters refer to the %s file found in the %s directory of this plugin. This file is the starting point for developing themes with Theme Mode.', 'option-tree' ), '<code>demo-functions.php</code>', '<code>/assets/theme-mode/</code>' ) . '</p>';
        
        echo '<p class="aside">' . __( 'You now have Theme Option built into your theme and anytime an update is available replace the old version with the new one.', 'option-tree' ) . '</p>';
...
require( trailingslashit( get_template_directory() ) . \'includes/theme-options.php\' );
        ...
        echo '<p>' . __( 'The code below is a boilerplate to get your started. For a full list of the available option types click the "Option Types" tab above. Also a quick note, you don\'t need to put Theme Option in theme mode to manually create options but you will want to hide the docs and settings as each time you load the admin area the settings be written over with the code below if they\'ve changed in any way. However, this ensures your settings do not get tampered with by the end-user.', 'option-tree' ) . '</p>';
